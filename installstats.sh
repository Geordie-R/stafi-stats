...
if [ ! -f "$config_file" ]
then
  echo "#Configuration file for the stafi node statistics script" > "$config_file"
...
#----------------------------------------------------------------------------------------------------#
  ...
  then
    ...
    not_setup=1
    read -p "TYPE THE NAME OF YOUR STAFI NODE: " -e node_name
...
#----------------------------------------------------------------------------------------------------#
  ...
  then
    ...
    not_setup=1
    read -p "WHAT IS THE MAXIMUM AMOUNT OF BLOCKS DIFFERENCE BETWEEN YOUR VALIDATOR AND STAFIS VALIDATOR YOU WILL ALLOW BEFORE BEING NOTIFIED. ALLOW FOR SLIGHT LAG e.g. 4: " -e block_diff_threshold
...
then
  ...
  (crontab -l 2>/dev/null; echo "*/5 * * * * $run_file") | crontab -
  echo "Added cron job to schedule the running of stats.sh every 5 minutes. Use crontab -e to view or amend"
fi
